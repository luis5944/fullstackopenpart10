...
describe("RepositoryList", () => {
  describe("RepositoryListContainer", () => {
    it("renders repository information correctly", () => {
      ...
      for (let i = 0; i < repositories.repositories.edges.length; i++) {
        expect(getAllByTestId("fullName")[i]).toHaveTextContent(
          repositories.repositories.edges[i].node.fullName
        );
        expect(getAllByTestId("description")[i]).toHaveTextContent(
          repositories.repositories.edges[i].node.description
        );
        expect(getAllByTestId("language")[i]).toHaveTextContent(
          repositories.repositories.edges[i].node.language
        );
        expect(getAllByTestId("reviewCount")[i]).toHaveTextContent(
          repositories.repositories.edges[i].node.reviewCount
        );
        expect(getAllByTestId("ratingAverage")[i]).toHaveTextContent(
          repositories.repositories.edges[i].node.ratingAverage
        );
      }
    });
  });
